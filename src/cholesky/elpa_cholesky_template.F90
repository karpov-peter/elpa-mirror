...
#include "../general/precision_kinds.F90"
  ...
  if (error .ne. ELPA_OK) then
    write(error_unit,*) "Problem getting size of mpi_comm_rows in cholesky. Aborting..."
    stop
  endif

  call obj%get("process_row", my_prow, error)
  if (error .ne. ELPA_OK) then
    write(error_unit,*) "Problem getting rank of mpi_comm_rows in cholesky. Aborting..."
    stop
  endif

  call obj%get("num_process_cols", np_cols, error)
  if (error .ne. ELPA_OK) then
    write(error_unit,*) "Problem getting size of mpi_comm_cols in cholesky. Aborting..."
    stop
  endif

  call obj%get("process_col", my_pcol, error)
  if (error .ne. ELPA_OK) then
    write(error_unit,*) "Problem getting rank of mpi_comm_cols in cholesky. Aborting..."
    stop
  endif
